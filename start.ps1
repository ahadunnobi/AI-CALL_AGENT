...
if (-not $SkipOllama) {
    ...
    if (Check-Command "ollama") {
        ...
        Write-Host "  WARNING: Ollama not in PATH. Start it manually or set OLLAMA_URL in .env."
    }
}
...
Write-Host "  AI Bridge starting (PID: $($PythonProc.Id))..." -ForegroundColor DarkGray

# Wait for server to be READY (Vosk can take 15+ seconds)
Write-Host "  Waiting for AI models to load (this can take 20s)..." -ForegroundColor Yellow
$MaxRetries = 30
$RetryCount = 0
while ($RetryCount -lt $MaxRetries) {
    try {
        ...
        if ($resp.status -eq "ok") {
            Write-Host "  AI Bridge is ONLINE! ✓" -ForegroundColor Green
            break
        }
    }
    catch {
    ...
    if ($RetryCount -eq $MaxRetries) {
        Write-Host "ERROR: AI Bridge failed to start. Check logs\python_server_err.log" -ForegroundColor Red
...
Write-Host "Press Ctrl+C faaaaaaaaahto exit."
